let peliculas = [
  {
    ...
    genre_ids: [878, 53, 18],
    overview:
      "El viaje de un hombre hacia el futuro para salvar un mundo moribundo.",
...
/**
 * Función que retorna la cantidad  de peliculas en el idioma ingles "en"
 * Hint: filter
 */

const peliculaIngles = () => {
  let arregloDeLasPelis = peliculas.filter((pelicula) => {
    if (pelicula["original_language"] === "en") {
      return pelicula;
    }
  });
  return arregloDeLasPelis.length;
};
console.log(`Las pelis en ingles es ${peliculaIngles()}`);

/**
 * Función que imprime los géneros que tienen una pelicula.
 * Como, parámetro, pasar el título (original_title de la película)
 * @param {*} title
 */
const printGenesByTitleMovie = (title) => {
  ...
  if (objPelicula) {
    objPelicula.genre_ids.forEach((id) => {
      const objgenero = generos.forEach((gen) => {
        if (gen.id === id) {
          return gen;
        }
      });
      if (objgenero) {
        console.log(`El genero de la peli es ${objgenero.name}`);
      }
    });
  }
};

printGenesByTitleMovie("After We Collided");
...
const mayorYMenor = () => {
    let mayor = 0;
    let pelisMayor = "";
    let menor = peliculas[0].vote_count;
    let pelisMenor = "";
    peliculas.forEach((peli)=>{
        if(peli.vote_count > mayor){
            mayor = peli.vote_count;
            pelisMayor = peli.original_title;
        }
    })
    console.log(`La pelicula preferida es ${pelisMayor} con ${mayor} votos`)
    peliculas.forEach((pelimenor)=>{
        if(pelimenor.vote_count < menor){
            pelisMenor = pelimenor.original_title;  
            menor = pelisMenor.vote_count;
        }
    })
    console.log(`La pelicula menos votada es ${pelisMenor} con ${menor} votos`)
}
mayorYMenor();
...
const mayorCantidadDeGeneros = () => {
    let mayor = 0;
    peliculas.forEach((peli)=>{
        if(peli.genre_ids.length > mayor)
        mayor = peli.genre_ids.length;
    })
    const pelisMayorCantidad = peliculas.filter((pelis)=>{
        if(pelis.genre_ids.length === mayor){
            return pelis
        }
    })
    console.log(pelisMayorCantidad)
}
mayorCantidadDeGeneros();   
...
const peliculasConGeneros = () => {
    const arregloNuevo = peliculas.map((pelis)=> {
       pelis.generos = [];
       pelis.genre_ids.forEach((ids)=>{
            generos.find((gen)=>{
               if(gen.id === ids){
                   pelis.generos.push(gen.name);
               }
           })
        })
        return pelis
...
console.log(peliculasConGeneros())
